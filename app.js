...
async function loadDataTabel(jenis) {
    ...
    else if (jenis === 'jenis-surat') actionName = 'getJenisSurat';
    else if (jenis === 'user') actionName = 'getUser';
    ...
    try {
        ...
        tbody.innerHTML = `<tr><td colspan="7" style="text-align: center; color: var(--danger);">Gagal memuat.</td></tr>`;
    }
}

function renderHTMLTabel(jenis, dataArray, tbody) {
    if (!dataArray || dataArray.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" style="text-align: center; padding:20px;">Belum ada data tersedia.</td></tr>`;
        return;
    }
    let html = '';
    ...
    dataArray.reverse().forEach(item => {
        ...
        if (jenis === 'jenis-surat') {
            html += `<tr><td><strong>${item.kode}</strong></td><td>${item.nama}</td><td>${item.uraian}</td>
                ...
                <button class="btn-icon text-danger" onclick="deleteData('deleteJenisSurat', '${item.id}', 'jenis-surat')"><i class="fa-solid fa-trash"></i></button></td></tr>`;
        } else if (jenis === 'user') {
...
const getBase64 = (file) => new Promise((resolve, reject) => {
    ...
    r.onload = () => resolve({ mimeType: file.type, filename: file.name, base64Data: r.result.split(',')[1] });
