...
let globalDataJenisSurat = []; let globalDataUser = []; let globalDataRefPK = []; let globalDataCabang = [];
...
function navigate(page) {
    ...
    const titles = { 'dashboard': 'Dashboard', 'surat-masuk': 'Surat Masuk', 'surat-keluar': 'Surat Keluar', 'disposisi': 'Disposisi', 'sppk': 'Data SPPK', 'pk': 'Data PK', 'arsip': 'Arsip Dokumen', 'laporan': 'Laporan', 'pengaturan': 'Pengaturan Sistem' }; document.getElementById('page-title').innerText = titles[page] || 'Aplikasi';
}
...
async function loadDataTabel(jenis) {
    ...
    let actionName = '';
    if (jenis === 'surat-masuk') actionName = 'getSuratMasuk'; else if (jenis === 'surat-keluar') actionName = 'getSuratKeluar'; else if (jenis === 'sppk') actionName = 'getSPPK'; else if (jenis === 'pk') actionName = 'getPK'; else if (jenis === 'disposisi') actionName = 'getDisposisi'; else if (jenis === 'arsip') actionName = 'getArsip'; else if (jenis === 'jenis-surat') actionName = 'getJenisSurat'; else if (jenis === 'user') actionName = 'getUser'; else if (jenis === 'referensi-pk') actionName = 'getReferensiPK'; else if (jenis === 'cabang') actionName = 'getCabang';
...
function renderHTMLTabel(jenis, dataArray, tbody) {
    ...
    let html = '';
    if(jenis === 'jenis-surat') globalDataJenisSurat = [...dataArray]; if(jenis === 'user') globalDataUser = [...dataArray]; if(jenis === 'referensi-pk') globalDataRefPK = [...dataArray]; if(jenis === 'cabang') globalDataCabang = [...dataArray];
    ...
    dataArray.reverse().forEach(item => {
        ...
        if (jenis === 'cabang') html += `<tr><td><strong>${item.nama}</strong></td><td>${item.kodeSM}</td><td>${item.kodePK}</td><td><button class="btn-icon text-primary" onclick="editCabang('${item.id}')"><i class="fa-solid fa-edit"></i></button> <button class="btn-icon text-danger" onclick="deleteData('deleteCabang', '${item.id}', 'cabang')"><i class="fa-solid fa-trash"></i></button></td></tr>`;
        else if (jenis === 'jenis-surat') html += `<tr><td><strong>${item.kode}</strong></td><td>${item.nama}</td><td>${item.uraian}</td><td><button class="btn-icon text-primary" onclick="editJenisSurat('${item.id}')"><i class="fa-solid fa-edit"></i></button> <button class="btn-icon text-danger" onclick="deleteData('deleteJenisSurat', '${item.id}', 'jenis-surat')"><i class="fa-solid fa-trash"></i></button></td></tr>`;
        else if (jenis === 'user') html += `<tr><td><strong>${item.username}</strong></td><td>${item.nama}</td><td>${item.role}</td><td>${item.jabatan}</td><td><button class="btn-icon text-primary" onclick="editUser('${item.id}')"><i class="fa-solid fa-edit"></i></button> <button class="btn-icon text-danger" onclick="deleteData('deleteUser', '${item.id}', 'user')"><i class="fa-solid fa-trash"></i></button></td></tr>`;
        else if (jenis === 'referensi-pk') html += `<tr><td><span class="badge badge-primary">${item.kategori}</span></td><td><strong>${item.kode}</strong></td><td>${item.uraian}</td><td><button class="btn-icon text-primary" onclick="editReferensiPK('${item.id}')"><i class="fa-solid fa-edit"></i></button> <button class="btn-icon text-danger" onclick="deleteData('deleteReferensiPK', '${item.id}', 'referensi-pk')"><i class="fa-solid fa-trash"></i></button></td></tr>`;
...
function editCabang(id) { const data = globalDataCabang.find(d => d.id === id); if(data) { document.getElementById('idCabang').value = data.id; document.getElementById('namaCabang').value = data.nama; document.getElementById('kodeSMSK').value = data.kodeSM; document.getElementById('kodePK').value = data.kodePK; document.getElementById('title-cabang').innerHTML = '<i class="fa-solid fa-edit text-primary"></i> Edit Cabang'; openModal('modal-cabang'); } }
...
function populatePKForm() {
    fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'getSPPK' }) }).then(res => res.json()).then(result => {
        if(result.status === 'success') {
            let options = '<option value="">Pilih SPPK...</option>'; result.data.forEach(j => { if(j.status !== "Sudah PK") options += `<option value="${j.nomorSPPK}">${j.nomorSPPK} - ${j.debitur}</option>`; }); document.getElementById('select-sppk-induk').innerHTML = options;
        }
    });
...
function handleLogin(e) {
    ...
    // Auto Load Master Cabang dan Jenis Surat ke Form Global
    fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'getCabang' }) }).then(res => res.json()).then(result => {
        if(result.status === 'success') {
            let options = '<option value="">Pilih Cabang...</option>';
            result.data.forEach(j => options += `<option value="${j.kodeSM}|${j.kodePK}">${j.nama}</option>`);
            document.querySelectorAll('.sel-cabang-global').forEach(el => el.innerHTML = options);
        }
    });

    fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'getJenisSurat' }) }).then(res => res.json()).then(result => {
        if(result.status === 'success') {
            let options = '<option value="">Pilih Jenis Surat...</option>'; result.data.forEach(j => options += `<option value="${j.kode}">${j.kode} - ${j.nama}</option>`);
            if(document.getElementById('select-jenis-sm')) document.getElementById('select-jenis-sm').innerHTML = options; if(document.getElementById('select-jenis-sk')) document.getElementById('select-jenis-sk').innerHTML = options;
        }
    });
}
