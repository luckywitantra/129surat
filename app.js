...
let storeData = { 'surat-masuk': [], 'surat-keluar': [], 'sppk': [], 'pk': [], 'arsip': [], 'cabang': [], 'jenis-surat': [], 'referensi-pk': [], 'user': [] };
...
async function initSystem() {
    ...
    } catch (error) { setTimeout(() => { document.getElementById('init-screen').classList.add('hidden'); document.getElementById('login-screen').classList.remove('hidden'); showAlert('Offline', 'Tanpa koneksi backend.', 'info'); }, 1000); }
}
...
function navigate(page) {
    ...
    const titles = { 'dashboard': 'Dashboard', 'surat-masuk': 'Surat Masuk', 'surat-keluar': 'Surat Keluar', 'sppk': 'Data SPPK', 'pk': 'Data PK', 'arsip': 'Arsip Dokumen', 'laporan': 'Laporan', 'pengaturan': 'Pengaturan Sistem' }; document.getElementById('page-title').innerText = titles[page] || 'Aplikasi';
}
...
function buildFilterUI(jenis) {
    ...
    container.innerHTML = `
        <div style="display:flex; gap:10px; margin-bottom:15px; flex-wrap:wrap; background:var(--bg-main); padding:15px; border-radius:8px; border:1px solid var(--border-color);">
            <input type="text" id="search-${jenis}" class="search-input" placeholder="Cari..." onkeyup="applyFilter('${jenis}')" style="flex:1; min-width:200px;">
...
function populateCabangFilters() { let ops = '<option value="">Semua Cabang</option>'; storeData['cabang'].forEach(c => ops += `<option value="${c.kodeSM}">${c.nama}</option>`); document.querySelectorAll('.sel-cabang-filter').forEach(el => { el.innerHTML = ops; }); }
...
async function loadDataTabel(jenis) {
    ...
    try {
        const response = await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: act }) }); const result = await response.json();
        if (result.status === 'success') { storeData[jenis] = result.data; if(jenis === 'cabang') populateCabangFilters(); if(jenis === 'pk') populatePKForm(); applyFilter(jenis); } 
...
function applyFilter(jenis) {
    if(!storeData[jenis]) return; let data = [...storeData[jenis]]; const tbody = document.getElementById(`tbody-${jenis}`); if(!tbody) return;

    if(['surat-masuk','surat-keluar','sppk','pk','arsip'].includes(jenis)) {
        const sv = document.getElementById(`search-${jenis}`) ? document.getElementById(`search-${jenis}`).value.toLowerCase() : '';
        const srt = document.getElementById(`sort-${jenis}`) ? document.getElementById(`sort-${jenis}`).value : 'newest';
        const cb = document.getElementById(`fil-cabang-${jenis}`) ? document.getElementById(`fil-cabang-${jenis}`).value : '';
        const bln = document.getElementById(`fil-bulan-${jenis}`) ? document.getElementById(`fil-bulan-${jenis}`).value : '';
        const thn = document.getElementById(`fil-tahun-${jenis}`) ? document.getElementById(`fil-tahun-${jenis}`).value : '';

        data = data.filter(item => {
            let mTxt = true, mCb = true, mBln = true, mThn = true;
            let target = (item.nomor||item.nomorSPPK||item.nomorPK||'') + " " + (item.pengirim||item.tujuan||item.debitur||item.deskripsi||'');
            if(sv) mTxt = target.toLowerCase().includes(sv);
            if(cb && item.cabang) mCb = item.cabang.includes(cb);
            if(item.tanggal) { const d = new Date(item.tanggal); if(bln) mBln = ("0"+(d.getMonth()+1)).slice(-2) === bln; if(thn) mThn = d.getFullYear().toString() === thn; }
            return mTxt && mCb && mBln && mThn;
        });
        ...
        data.sort((a, b) => {
            ...
            if(srt === 'newest') return db - da; if(srt === 'oldest') return da - db;
            if(srt === 'az') return txtA.localeCompare(txtB); if(srt === 'za') return txtB.localeCompare(txtA);
        });
    }

    let html = '';
    if (data.length === 0) { tbody.innerHTML = `<tr><td colspan="7" style="text-align: center; padding:20px;">Belum ada data tersedia.</td></tr>`; return; }
    ...
    data.forEach(item => {
        ...
        let s = (item.status || "").toLowerCase(); let badge = 'warning'; if(s.includes('selesai')||s.includes('terkirim')||s.includes('sudah')||s.includes('aktif')) badge = 'success'; if(s.includes('belum')) badge = 'danger'; let statusBadge = `<span class="badge badge-${badge}">${item.status || '-'}</span>`;
...
function showTimeline(debitur) {
    ...
    document.getElementById('timeline-debitur').innerText = `Riwayat SLA: ${debitur}`; document.getElementById('timeline-content').innerHTML = tHtml; openModal('modal-timeline');
}
...
function editData(jenis, id) {
    ...
    } else if (jenis === 'cabang') editCabang(id); else if (jenis === 'jenis-surat') editJenisSurat(id); else if (jenis === 'referensi-pk') editReferensiPK(id); else if (jenis === 'user') editUser(id);
}
...
async function sendFormData(action, payload, formEl, modalId, jenisMenuRef) { const btn = formEl.querySelector('button[type="submit"]'); const originalBtnHTML = btn.innerHTML; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Memproses...'; btn.disabled = true; try { const response = await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: action, payload: payload }) }); const result = await response.json(); if (result.status === 'success') { if(modalId) closeModal(modalId); showAlert('Berhasil', result.message, 'success'); if(modalId) formEl.reset(); if(jenisMenuRef) loadDataTabel(jenisMenuRef); if(jenisMenuRef && !['user','jenis-surat','referensi-pk','cabang'].includes(jenisMenuRef)) loadDashboardStats(); } else showAlert('Gagal', result.message, 'error'); } catch (error) { showAlert('Koneksi Gagal', 'Gagal mengirim data.', 'error'); } finally { btn.innerHTML = originalBtnHTML; btn.disabled = false; } }
...
function populatePKForm() {
    let options = '<option value="">Pilih SPPK...</option>'; storeData['sppk'].forEach(j => { if(j.status !== "Sudah PK") options += `<option value="${j.nomorSPPK}">${j.nomorSPPK} - ${j.debitur}</option>`; }); document.getElementById('select-sppk-induk').innerHTML = options;
    ...
    fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'getReferensiPK' }) }).then(res => res.json()).then(result => { if(result.status === 'success') { let opsGol = '<option value="">Pilih...</option>', opsJns = '<option value="">Pilih...</option>', opsKlas = '<option value="">Pilih...</option>', opsSek = '<option value="">Pilih...</option>'; result.data.forEach(j => { let txt = `<option value="${j.kode}">${j.kode} - ${j.uraian}</option>`; if(j.kategori === 'GolDebitur') opsGol += txt; else if(j.kategori === 'JnsPenggunaan') opsJns += txt; else if(j.kategori === 'KlasKredit') opsKlas += txt; else if(j.kategori === 'SektorEko') opsSek += txt; }); document.getElementById('sel-goldebitur').innerHTML = opsGol; document.getElementById('sel-jnspenggunaan').innerHTML = opsJns; document.getElementById('sel-klaskredit').innerHTML = opsKlas; document.getElementById('sel-sektoreko').innerHTML = opsSek; } });
}
...
function editCabang(id) { const data = storeData['cabang'].find(d => d.id === id); if(data) { document.getElementById('idCabang').value = data.id; document.getElementById('namaCabang').value = data.nama; document.getElementById('kodeSMSK').value = data.kodeSM; document.getElementById('kodePK').value = data.kodePK; document.getElementById('title-cabang').innerHTML = '<i class="fa-solid fa-edit text-primary"></i> Edit Cabang'; openModal('modal-cabang'); } }
